export const DataDisplayRow = ({
	...
	return (
		<div className={rowClass}>
			...
			{dougnnutChart && (
				<dd className="mt-1 text-sm text-gray-900  sm:mt-0 col-span-4 sm:col-start-3 sm:col-end-6 flex justify-center">
					<div className="w-2/3">{dougnnutChart}</div>
				</dd>
			)}
			{table && (
				<dd className=" text-sm text-gray-900  col-span-6 ">{table}</dd>
			)}
			{actionsBar && (
				<dd className=" text-sm text-gray-900  col-span-6 ">{actionsBar}</dd>
			)}
			{buyBtn}
			{backBtn}
		</div>
